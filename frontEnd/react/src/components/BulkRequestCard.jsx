...
// Botão para as abas de modo
const ModeButton = ({ children, onClick, isActive }) => (
  <button
    type="button"
    onClick={onClick}
    className={`px-4 py-2 text-sm font-semibold rounded-md flex items-center space-x-2 transition-colors duration-200 ${
      isActive
...
const BulkRequestCard = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (mode === 'list') {
      ...
      try {
        ...
        setResults([{ id: 'file-error', cpf: 'Erro de Arquivo', status: 'error', error: 'Não foi possível ler o conteúdo do arquivo.' }]);
        setIsLoading(false);
        return;
      }
    }
    
    // Adiciona o campo 'isOpen' para controlar o detalhe de cada resultado
    const initialResults = cpfList.map((cpf, index) => ({ id: `${cpf}-${index}`, cpf, status: 'pending', isOpen: false }));
    setResults(initialResults);

    // Dispara as requisições em paralelo, mas o navegador limita a concorrência
    const promises = cpfList.map(cpf => getScore(cpf, clientId));

    // Processa os resultados um por um, atualizando a UI a cada resposta
  ...
  // Função para abrir/fechar os detalhes de um resultado
  ...
  return (
    <div className="bg-black border-2 border-green-600 rounded-xl shadow-2xl p-6 sm:p-8 col-span-1 md:col-span-2">
      <div className="flex items-center space-x-3 mb-1">
        ...
        <h2 className="text-2xl font-bold text-green-400">Gerador de Rajadas (Consulta em Massa)</h2>
      </div>
      <p className="text-green-400 mb-6">Selecione um método para enfileirar múltiplas requisições e testar o proxy.</p>
            ...
            {results.map((result) => (
              <li key={result.id} className="bg-gray-900/50 rounded-md transition-all duration-300">
                <button
                  ...
                  className="w-full flex items-center justify-between text-sm p-2 text-left disabled:cursor-not-allowed"
                >
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    {result.status !== 'pending' && (
                      <ChevronDown
                        className={`text-gray-400 transition-transform duration-200 ${result.isOpen ? 'rotate-180' : ''}`}
                        size={16}
                      />
                    )}
                  </div>
                ...
                {result.isOpen && (
                  <div className="p-2 border-t border-green-600/20">
                    {result.status === 'success' && (
                      <pre className="text-xs text-green-200 bg-black p-2 rounded overflow-auto">{JSON.stringify(result.data, null, 2)}</pre>
                    )}
                    {result.status === 'error' && (
